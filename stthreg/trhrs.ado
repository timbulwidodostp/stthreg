...
local names : colfullnames e(b)
   ...
   while trim(`"`colnames_all_remain'"')!=""{
	    ...
	    local is_interesting_dummy = 0
		   ...
		   di "s_c_`current_eb_covariate':`s_c_`current_eb_covariate''"
   ...
   *end while trim(`"`colnames_all_remain'"')!=""{     
...
if("`ci'"==""){
	   ...
	   foreach _timevalue of local timevalue{
		   ...
		   local f0=exp((ln(`_y0')-.5*(ln(2*_pi*(`_timevalue'^3))+(`_y0'+`_mu'*`_timevalue')^2/`_timevalue')))


		   local S0=exp(ln(normal((`_mu'*`_timevalue'+`_y0')/sqrt(`_timevalue'))/*
											 */ -exp(-2*`_y0'*`_mu')*normal((`_mu'*`_timevalue'-`_y0')/sqrt(`_timevalue'))))
		   ...
		   foreach dummy_var_interest of varlist  `dummy_list' {
			   ...
			   local f_`dummy_var_interest'=exp((ln(`_y0_`dummy_var_interest'')-.5*(ln(2*_pi*(`_timevalue'^3))+(`_y0_`dummy_var_interest''+`_m_`dummy_var_interest''*`_timevalue')^2/`_timevalue')))

			   local S_`dummy_var_interest'=exp(ln(normal((`_m_`dummy_var_interest''*`_timevalue'+`_y0_`dummy_var_interest'')/sqrt(`_timevalue'))/*
																			       */ -exp(-2*`_y0_`dummy_var_interest''*`_m_`dummy_var_interest'')*normal((`_m_`dummy_var_interest''*`_timevalue'-`_y0_`dummy_var_interest'')/sqrt(`_timevalue'))))
		...
		foreach dummy_var_interest of varlist  `dummy_list' {
			   ...
			   local line_command `line_command' (connected `hrv_`dummy_var_interest'' `hr_t', sort ) 
			      ...
			      graph twoway `line_command',name(HazardRatio`dummy_var_interest',replace) subtitle(`"``vlb_`dummy_var_interest''' v.s. ``vlbl_reference''"') ytitle("Estimated Hazard Ratio")
                              ...
                              graph twoway `line_command',name(HazardRatio`dummy_var_interest',replace) `graphopt'
			   }
   ...
   local last_para "`e(stthreg_para)'"
   ...
   foreach _timevalue of local timevalue{
	   ...
	   local bc_h0 (exp((ln(exp((`lny0_reference_bc')))-.5*(ln(2*_pi*(`_timevalue'^3))+(exp((`lny0_reference_bc'))+(`mu_reference_bc')*`_timevalue')^2/`_timevalue'))))/(exp(ln(normal(((`mu_reference_bc')*`_timevalue'+exp((`lny0_reference_bc')))/sqrt(`_timevalue'))/*
     */ -exp(-2*exp((`lny0_reference_bc'))*(`mu_reference_bc'))*normal(((`mu_reference_bc')*`_timevalue'-exp((`lny0_reference_bc')))/sqrt(`_timevalue')))))
           *di "bc_h0:`bc_h0'"
	   ...
	   foreach dummy_var_interest of varlist  `dummy_list' {
			   ...
			   while trim(`"`colnames_all_remain'"')!=""{
			    ...
			    local b_h_`dummy_var_interest' (exp((ln(exp((`l_b_`dummy_var_interest'')))-.5*(ln(2*_pi*(`_timevalue'^3))+(exp((`l_b_`dummy_var_interest''))+(`m_b_`dummy_var_interest'')*`_timevalue')^2/`_timevalue'))))/(exp(ln(normal(((`m_b_`dummy_var_interest'')*`_timevalue'+exp((`l_b_`dummy_var_interest'')))/sqrt(`_timevalue'))/*
     */ -exp(-2*exp((`l_b_`dummy_var_interest''))*(`m_b_`dummy_var_interest''))*normal(((`m_b_`dummy_var_interest'')*`_timevalue'-exp((`l_b_`dummy_var_interest'')))/sqrt(`_timevalue')))))

			    di ""
			    di ""
			    trbootstrap hr=((`b_h_`dummy_var_interest'')/(`bc_h0')), reps(`bootstrap') level(`level'): stthreg `last_para'
   ...
   if ("`graph'"=="hz" | "`graph'"=="sv" | "`graph'"=="ds"){
	   ...
	   foreach dummy_var_interest of varlist  `dummy_list' {
        ...
        qui gen `var_f0'=exp((ln(`_y0')-.5*(ln(2*_pi*(`var_time'^3))+(`_y0'+`_mu'*`var_time')^2/`var_time')))
  
        qui gen `var_S0'=exp(ln(normal((`_mu'*`var_time'+`_y0')/sqrt(`var_time'))/*
                */ -exp(-2*`_y0'*`_mu')*normal((`_mu'*`var_time'-`_y0')/sqrt(`var_time'))))
        ...
        if("`graph'"=="hz"){    
				 ...
				 local graphoption_command `"title(Hazard Function) ytitle(Estimated Hazard Function) name(hazard,replace)"'
				 local name_command `"name(hazard,replace)"'
        }
        else if ("`graph'"=="sv"){
				 label variable `var_S0' `"``vlbl_reference''"'
				 local line_command `var_S0' `var_time'     
				 local graphoption_command `" title(Survival Function) ytitle(Estimated Survival Function) name(survival,replace)"'
				 local name_command `"name(survival,replace)"'
        }
        else if ("`graph'"=="ds"){
				 label variable `var_f0' `"``vlbl_reference''"'
				 local line_command `var_f0' `var_time'        
				 local graphoption_command `"title(Probability Density Function) ytitle(Estimated Probability Density Function) name(density,replace)"'
